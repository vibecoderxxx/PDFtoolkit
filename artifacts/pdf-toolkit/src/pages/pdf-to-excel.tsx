...
export default function PdfToExcel() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/pdf/pdf-to-excel", { method: "POST", body: formData });
      if (!response.ok) throw new Error("Failed");
      const blob = await response.blob();
      setDownloadUrl(URL.createObjectURL(blob));
      toast({ title: "Success!", description: "PDF converted to Excel." });
    } catch {
    ...
    <Layout>
      <div className="container mx-auto px-4 py-12 max-w-4xl">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold tracking-tight text-foreground mb-3">PDF to EXCEL</h1>
          <p className="text-muted-foreground text-lg">Extract data from your PDF files into Excel spreadsheets.</p>
        </div>
        {!downloadUrl ? (
          <div className="space-y-8 bg-card p-6 md:p-8 rounded-2xl shadow-sm border">
            {!file ? (
              <FileUpload onUpload={handleUpload} title="Select PDF file" description="Upload a PDF to convert to Excel" />
            ) : (
              <div className="space-y-6">
                ...
                <div className="pt-4 flex justify-center">
                  <Button size="lg" className="min-w-[200px] shadow-md" onClick={handleConvert} disabled={isProcessing}>
                    {isProcessing ? <><Loader2 className="w-5 h-5 mr-2 animate-spin" />Converting...</> : "Convert to Excel"}
                  </Button>
                </div>
              </div>
            )}
          ...
          <div className="bg-card p-10 rounded-2xl shadow-sm border text-center space-y-6">
            ...
            <h2 className="text-2xl font-bold">Your Excel file is ready!</h2>
            <div className="flex justify-center space-x-4 pt-4">
              <Button size="lg" onClick={() => { const a = document.createElement("a"); a.href = downloadUrl; a.download = "data.xlsx"; a.click(); }} className="shadow-md">Download XLSX</Button>
