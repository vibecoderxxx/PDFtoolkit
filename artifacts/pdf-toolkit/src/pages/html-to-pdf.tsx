...
export default function HtmlToPdf() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      toast({ title: "Success!", description: "HTML converted to PDF." });
    } catch {
    ...
    <Layout>
      <div className="container mx-auto px-4 py-12 max-w-4xl">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold tracking-tight text-foreground mb-3">HTML to PDF</h1>
          <p className="text-muted-foreground text-lg">Convert HTML files to PDF documents.</p>
        </div>
        {!downloadUrl ? (
          <div className="space-y-8 bg-card p-6 md:p-8 rounded-2xl shadow-sm border">
            {!file ? (
              <FileUpload onUpload={handleUpload} accept={{ "text/html": [".html", ".htm"] }} title="Select HTML file" description="Upload an HTML file" />
            ) : (
          ...
          <div className="bg-card p-10 rounded-2xl shadow-sm border text-center space-y-6">
            ...
            <div className="flex justify-center space-x-4 pt-4">
              <Button size="lg" onClick={() => { const a = document.createElement("a"); a.href = downloadUrl; a.download = "page.pdf"; a.click(); }} className="shadow-md">Download PDF</Button>
